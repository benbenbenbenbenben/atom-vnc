  ...
  class VncView {
    ...
    mouseWheel(event, element) {
      ...
      if (delta > 0) {
        this.connection.pointerEvent(x, y, this.buttonStateBitset | 1 << 3);
      } else {
        this.connection.pointerEvent(x, y, this.buttonStateBitset | 1 << 4);
      }
      this.connection.pointerEvent(x, y, this.buttonStateBitset);
    ...
    captureCanvas(canvas, callback) {
      canvas.toBlob(function(blob) {
        ...
        reader.onloaded(function(res) {
          let arr = new Uint8Array(res.result)
          callback(arr)
        })
    ...
    captureFrameRegion(x, y, width, height, callback) {
      ...
      buffer.getContext('2d').drawImage(this.drawable, x, y, w, h, 0, 0, w, h)
