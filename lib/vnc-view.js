  ...
  class VncView {
    ...
    initialize(params = {uri:"http://127.0.0.1:5900"}) {
      ...
      this.drawable.addEventListener("mousewheel", (e, l) => this.mouseDown.bind(this)(e, this.drawable))
    ...
    mouseMove(event, element) {
      ...
      x = event.pageX - (element.offsetLeft * this.scale);
      y = event.pageY - (element.offsetTop * this.scale);
    ...
    mouseWheel(event, element) {
      ...
      x = event.pageX - _this.drawable.offsetLeft;
      y = event.pageY - _this.drawable.offsetTop;
      delta = event.wheelDelta;
      if (delta > 0) {
        this.connection.pointerEvent(x, y, _this.buttonStateBitset | 1 << 3);
      } else {
        this.connection.pointerEvent(x, y, _this.buttonStateBitset | 1 << 4);
      }
      this.connection.pointerEvent(x, y, _this.buttonStateBitset);
      return console.log('Wheel!', event.wheelDelta);
    }
    ...
    get scale() {
      ...
      if (transform && transform.length) {
        return parseFloat(/\d+\.?\d*/.exec(transform))
      }
      return 1.0
    }
    set scale(value) {
      this.drawable.style.transformOrigin = "top left"
